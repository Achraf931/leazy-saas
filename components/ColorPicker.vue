      ...
      <div class="flex flex-col p-2 w-full h-full">
        ...
        <div v-else class="flex items-center p-1 rd-1 cursor-pointer hover:bg-accent" @click="setColor(undefined)">
          <span class="w-6 h-6 p-0.5 inline-block rounded-sm border border-transparent cursor-pointer">
            <span :style="{ backgroundColor: DEFAULT_COLOR }" class="relative w-[18px] h-[18px] block rounded-[2px] border-transparent">
        ...
        <span class="flex p-0 w-full h-auto relative last:pb-2" v-for="(items, index) in chunkedColors" :key="index">
          <span
              ...
              @click="setColor(item)"
          >
            <span :style="{ backgroundColor: item }" class="relative w-[18px] h-[18px] block rounded-[2px] border-transparent">
              <svg v-if="item !== modelValue" viewBox="0 0 18 18" style="fill: rgb(255, 255, 255); display: none">
                <path d="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z"></path>
              </svg>
              <svg v-else class="absolute top-[-1px] left-[1px] w-3 h-3" viewBox="0 0 18 18" style="fill: rgb(255, 255, 255); display: block">
                <path d="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z"></path>
              </svg>
            </span>
          </span>
        ...
        <div>
          ...
          <span class="flex p-0 w-full h-auto relative last:pb-2">
            <span
                ...
                @click="setColor(item)"
            >
              <span class="relative w-[18px] h-[18px] block rounded-[2px] border-transparent" :style="{ backgroundColor: item }">
                <svg viewBox="0 0 18 18" style="fill: rgb(255, 255, 255); display: none">
                  <path d="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z"></path>
                </svg>
              </span>
