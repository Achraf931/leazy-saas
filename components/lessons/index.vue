...
watch(page, async (page) => {
  lessons.value = await client('/api/teacher/lessons', { params: { page } })
})
</script>

<template>
  <UDashboardToolbar :ui="{ wrapper: 'px-0 border-0' }">
    <template #left>
  ...
  <template v-if="!pending">
    <UBlogList v-if="filteredLessons.length" orientation="horizontal" :ui="{ wrapper: 'p-px overflow-y-auto gap-4 sm:grid sm:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5' }">
      <UBlogPost v-for="lesson in filteredLessons" :key="lesson.id" :to="localePath({ name: 'library-lessons-id', params: { id: lesson.id } })" :ui="{ wrapper: '' }" class="bg-white dark:bg-gray-900 ring-1 ring-gray-200 dark:ring-gray-800 shadow rounded-lg text-xs p-3">
    ...
    <div v-if="filteredLessons && lessons.last_page > 1" class="p-2.5 flex items-center justify-center">
