...
<template>
  ...
  >
    <div
      ...
      style="transition-property: top, left; transition-timing-function: ease-in-out; transition-duration: 0.2s"
    >
      <UButton variant="ghost" color="gray" @click="handleAdd" size="2xs" icon="i-heroicons-plus" />

      <UPopover v-model:open="menuOpen" :popper="{ placement: 'top-start' }" :ui="{ base: 'p-1' }">
        <UButton variant="ghost" color="gray" size="2xs" icon="i-lucide-grip-vertical" />
        ...
        <template #panel>
          <div class="w-max flex flex-col">
            ...
            <UButton variant="ghost" size="xs" color="gray" @click="resetTextFormatting" icon="i-lucide-paint-roller" :label="$t('editor.clear.tooltip')" />
            <UButton variant="ghost" size="xs" color="gray" @click="copyNodeToClipboard" icon="i-heroicons-clipboard" :label="$t('editor.copy')" />
            <UButton variant="ghost" size="xs" color="gray" @click="duplicateNode" icon="i-lucide-copy" :label="$t('editor.duplicate')" />

            <UDivider orientation="horizontal" class="my-1" />

            <UButton variant="soft" size="xs" color="red" @click="deleteNode" icon="i-heroicons-trash" :label="$t('editor.remove')" />
          </div>
        </template>
      </UPopover>
    </div>
