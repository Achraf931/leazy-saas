...
const { data: theme, refresh, error } = await useAsyncData('theme', async () => client(`/api/teacher/themes/${documentId.value}`))

if (error.value) await navigateTo(localePath({ name: 'library-themes' }))
...
const deleteChapter = async (id) => {
  await client(`/api/library/chapters/${id}`)
}
      ...
      <section class="flex-1 flex flex-col md:flex-row gap-5 p-px overflow-hidden w-full">
        <div class="flex flex-col gap-3">
          ...
          <div class="flex items-center justify-start gap-1">
            ...
            <p class="text-gray-400 text-xs">Créé {{ formatDistanceToNow(new Date(theme.created_at), { locale: frLocale, addSuffix: true }) }}</p>
          </div>
          <div class="flex items-center justify-start gap-1">
            <UIcon name="i-heroicons-clock" class="w-3.5 h-3.5 text-gray-400" />
            <p class="text-gray-400 text-xs">Modifié {{ formatDistanceToNow(new Date(theme.updated_at), { locale: frLocale, addSuffix: true }) }}</p>
          </div>
        </div>
