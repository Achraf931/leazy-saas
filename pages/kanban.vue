...
const updateTaskColumn = ({ task, column, id }) => {
            ...
            <div v-for="column in columns" :key="column.id" class="relative flex flex-col w-72">
                ...
                >
                  <template #item="{ element: task }: { element: Task }">
                      <KanbanTask :task="task" :column="column" :columns="columns" @update="updateTaskColumn" @delete="column.tasks = column.tasks.filter((t) => t.id !== $event)"/>
                  </template>
                </Draggable>
              </div>
          </div>
