...
<template>
  ...
  >
    <div v-for="(group, groupIndex) in items" :key="group.title" class="flex flex-col">
      <div class="text-neutral-500 text-[0.65rem] col-[1/-1] mx-2 mt-2 font-semibold tracking-wider pointer-events-none select-none uppercase first:mt-0.5">{{ $t(group.title) }}</div>
      <UButton
      ...
      >
        <template #leading>
          <div class="flex items-center justify-center p-px font-medium border rounded-sm border-gray-200 dark:border-gray-700">
            <UIcon v-if="command.iconName" :name="command.iconName" dynamic class="w-3.5 h-3.5 text-lg" />
          </div>
        </template>
        ...
        <div class="flex flex-1 items-center justify-between">
          ...
          <div class="flex items-center gap-0.5 ml-auto">
            <UKbd v-if="command.shortcuts?.length" v-for="(shortcut, index) in command.shortcuts" :key="index" size="xs" :value="shortcut" />
          </div>
        </div>
      </UButton>
    </div>
