    ...
    <div class="flex items-start justify-start gap-4 relative flex-1 overflow-hidden">
      <div class="flex flex-col gap-3 sticky top-0 left-0 max-w-[210px]">
        ...
        <div class="flex flex-col justify-start gap-2">
          ...
          <UCheckbox color="primary" v-model="selected.last" label="Terminal Production Graphique" :ui="{ wrapper: 'items-center overflow-hidden', container: 'h-auto', base: 'w-3 h-3', label: 'text-xs text-ellipsis truncate', inner: 'ms-1.5 truncate', border: 'border-primary' }" />
          <UCheckbox color="orange" v-model="selected.first" label="Seconde Production Graphique" :ui="{ wrapper: 'items-center overflow-hidden', container: 'h-auto', base: 'w-3 h-3', label: 'text-xs text-ellipsis truncate', inner: 'ms-1.5 truncate', border: 'border-orange-400' }" />
        </div>
      </div>
      ...
      >
        ...
        <template #arrow-next>
          <UButton size="2xs" icon="i-heroicons-chevron-right-20-solid" square color="white" />
        </template>
...
<!--                <UButton class="ml-auto" icon="i-heroicons-x-mark" size="2xs" color="gray" variant="ghost" @click="close" />
        ...
        <template #today-button>
          <UTooltip text="Aujourd'hui" :popper="{ placement: 'top' }">
...
.vuecal__event.break .vuecal__event-time {display: none;align-items: center;}
