...
import { defaultBubbleList, generateBubbleTypeMenu, type BubbleOptions } from '@/components/menus/bubble'
...
import { Lesson, type LessonOptions } from './Lesson'
import { LessonUpload, type LessonUploadOptions } from './LessonUpload'
...
import { Twitter, type TwitterOptions } from './Twitter'
import { TwitterUpload, type TwitterUploadOptions } from './TwitterUpload'
...
export const BaseKit = Extension.create<BaseKitOptions>({
  ...
  addExtensions() {
    ...
    if (this.options.lesson !== false) {
      extensions.push(Lesson.configure(this.options.lesson))
    }

    if (this.options.lessonUpload !== false) {
      extensions.push(LessonUpload.configure(this.options.lessonUpload))
    }
    ...
    if (this.options.twitter !== false) {
      extensions.push(Twitter.configure(this.options.twitter))
    }

    if (this.options.twitterUpload !== false) {
      extensions.push(TwitterUpload.configure(this.options.twitterUpload))
    }
